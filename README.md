...
> discovery, newest first. Last material update: **2026-09-20**. Everything is dated and measured on
...
- **A one-line `gfx1250`-only gate silently disabled flash-attention on gfx1201, and it looked like a compression bug.** TurboQuant KV quantization OOM'd at depth with no hint why — turned out `vllm/v1/attention/backends/fa_utils.py` only routes AITER's Triton flash-attn to `gfx1250`; every other ROCm arch, gfx1201 included, falls to an *uninstalled* upstream `flash_attn` package and loses flash-attention with zero startup error. The actual symptom was a dense `O(q_len·seq_len)` SDPA fallback materializing a full attention-score matrix — **4.6 GiB in one call at depth 32,000** — not anything about the KV compression itself. Three other bugs (a rejected KV dtype, a cudagraph/compile_sizes conflict, a hard MTP incompatibility) had to be cleared first to even find this one. ([15](docs/15-turboquant-kv-quant-gfx1201.md))
