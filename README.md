...
- **This model family can't be served inside its own spec on one card.** Qwen advises **≥128K context "to preserve thinking capabilities"** (Qwen3.6-27B card, *Serving*); a 27B at TP=1 on one 32 GB card tops out at **65,536** — half that — because the weights leave ~5 GiB for KV. The degeneration they warn about was then caught live: a repeating **~16k-token generation cycle every ~4 minutes for 57 minutes with zero agent progress**, ending in the 3-hour timeout. It's a probability gradient, not a cliff — Qwen ships a dial (`presence_penalty` 0–2) rather than a threshold — but it makes a second card a *correctness* argument, not a capacity one. ([13 §6e](docs/13-vllm-mxfp4-w4a8-rdna4.md))
