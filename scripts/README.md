...
| `swarm_desc.py` | The 60-worker image swarm: a fixed set of rendered images described concurrently by a thread pool against one serve, streaming responses, per-stream TTFT/wall/PP/TG plus aggregate, and all generated descriptions saved. The 134-image / 60-worker run behind [docs/14 §7](../docs/14-vllm-9b-mxfp4-60-agent-fanout.md). |
| `mtp_vs_ns.py` | Side-by-side printer for the MTP vs no-spec JSON pairs (ladders, rungs, depth profile) — the table in [docs/14 §4](../docs/14-vllm-9b-mxfp4-60-agent-fanout.md). Point `BENCH_DIR` at the results directory. |
